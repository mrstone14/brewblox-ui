...
<template>
  <svg v-bind="{ width, height }">
    <foreignObject v-bind="{ width, height }">
      <div v-if="editable || !url" class="fit column text-grey justify-center items-center q-gutter-y-md">
        <q-icon name="mdi-image-frame" size="xl" />
        <div class="col-auto text-center">
          {{ url || "No iframe URL set" }}
        </div>
      </div>
      <iframe v-else :src="url" referrerpolicy="no-referrer" allowfullscreen :style="{
        border: 'none',
    ...
    <BuilderInteraction v-if="editable" v-bind="{ width, height }">
      <q-menu touch-position context-menu>
        <q-list>
          ...
          <TextMenuContent :settings-key="URL_KEY" label="Edit URL"
            message="URL must include the http:// or https:// prefix." />
          <SliderMenuContent :min="10" :max="300" :default="100" :settings-key="WEBFRAME_SCALE_KEY" label="Adjust zoom"
            postfix="%" />
        </q-list>
      </q-menu>
    </BuilderInteraction>
